...
SCRIPT_L.writelines('cd $CMSSW_BASE/src/UserCode/HcalTPAnalyzer/; \n\n')
...
for proc in procList :
	...
	mytag = mytag.replace("+","")
        ...
        SCRIPT_Temp.writelines('cd $CMSSW_BASE/src/UserCode/HcalTPAnalyzer/; \n\n')
        ...
        for d in data :
            ...
            SCRIPT_DTag.writelines('cd $CMSSW_BASE/src/UserCode/HcalTPAnalyzer/; \n\n')

            # Loop over files for given dtag name:
            ntplpath = '/eos/cms/store/user/'+who+"/"+inputdir + '/crab_' + origdtag + '*/*/*/'

            segment=0
            for file in glob.glob(ntplpath+'analyze_tps_*.root'):
                print file
                eventsFile = file
#            eventsFile = ', \n '.join('"' + item + '"' for item in FileList)
         
                sedcmd = 'sed \"s%"@input"%' +eventsFile +'%;'
                sedcmd += 's%"@outdir"%' + outdir +'%;s%@isMC%' + str(not isdata) +'%;s%@xsec%'+str(xsec)+'%;'
...
os.system('cp $CMSSW_BASE/src/UserCode/HcalTPAnalyzer/scripts/splitlocaljobs.py '+queuelog+'/all/')
os.system('cp $CMSSW_BASE/src/UserCode/HcalTPAnalyzer/scripts/checkLocaljobs.py '+queuelog+'/all/')
